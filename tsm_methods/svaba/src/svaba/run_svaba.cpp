...
// output files
static ogzstream all_align, os_allbps, os_discordant, os_corrected, os_fpaout;
...
namespace opt {
  ...
  // fpa options
  static bool fpa = false;             // do fpa alignment
  static bool fpa_print = false;       // print fpa events
  static int fpa_ref_flank = 100;      // reference flank
//  static int fpa_qry_flank = 40;       // query flank
  static int fpa_frag_length = 10;     // (2)->(3) fragment minimum length
  static float fpa_flank_sim = 0.95;   // 10bp flanking sequence minimum identity
  static float fpa_frag_sim = 0.95;    // (2)->(3) fragment minimum identity
  static bool fpa_keep_masked = false; // keep masked regions (?)
  static int fpa_min_better = 2;       // minimum mismatch improvement over fwd alignment
  static bool fpa_debug = false;       // show debug info
...
static const struct option longopts[] = {
  ...
  { "fpa",                     no_argument, NULL, OPT_DO_FPA },
  { "fpa-print",               no_argument, NULL, OPT_FPA_PRINT },
  { "fpa-ref-flank",           required_argument, NULL, OPT_FPA_REF_FLANK },
  { "fpa-qry-flank",           required_argument, NULL, OPT_FPA_QRY_FLANK },
  { "fpa-frag-length",         required_argument, NULL, OPT_FPA_FRAG_LENGTH },
  { "fpa-flank-sim",           required_argument, NULL, OPT_FPA_FLANK_SIM },
  { "fpa-frag-sim",            required_argument, NULL, OPT_FPA_FRAG_SIM },
  { "fpa-keep-masked",         no_argument, NULL, OPT_FPA_KEEP_MASK },
  { "fpa-debug",               no_argument, NULL, OPT_FPA_DEBUG },
  { "fpa-min-better",          required_argument, NULL, OPT_FPA_MIN_BETTER },
  { NULL, 0, NULL, 0 }
};
...
"  FPA alignment params\n"
"      --fpa                            Do FPA alignment [false]\n"
"      --fpa-print                      Print FPA solution [false]\n"
"      --fpa-ref-flank                  FPA reference flanking sequence length [100]\n"
//"      --fpa-qry-flank                  FPA query flanking sequence length [40]\n"
"      --fpa-frag-length                FPA (2)->(3) fragment minimum length [10]\n"
"      --fpa-flank-sim                  FPA 10bp flanking sequence minimum identity [0.95]\n"
"      --fpa-frag-sim                   FPA (2)->(3) fragment minimum identity [0.95]\n"
//"      --fpa-keep-masked                FPA keep masked regions (?)\n"
"      --fpa-min-better                 FPA minimum mismatch improvement over forward alignment [2]\n"
"\n";
...
afterlearn: 
  ...
  if (opt::fpa)
    svabaUtils::fopen(opt::analysis_id + ".fpa.out.gz", os_fpaout);
...
void parseRunOptions(int argc, char** argv) {
  ...
  for (char c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1;) {
    ...
    case OPT_DO_FPA : opt::fpa = true; break;
    case OPT_FPA_PRINT : opt::fpa_print = true; break;
    case OPT_FPA_REF_FLANK : arg >> opt::fpa_ref_flank; break;
    case OPT_FPA_FRAG_LENGTH : arg >> opt::fpa_frag_length; break;
    case OPT_FPA_FLANK_SIM : arg >> opt::fpa_flank_sim; break;
    case OPT_FPA_FRAG_SIM : arg >> opt::fpa_frag_sim; break;
    case OPT_FPA_KEEP_MASK : opt::fpa_keep_masked = true; break;
    case OPT_FPA_MIN_BETTER : arg >> opt::fpa_min_better; break;
    case OPT_FPA_DEBUG : opt::fpa_debug = true; break;
      case 'c':
	tmp = "";
	arg >> tmp;
...
void run_assembly(const SeqLib::GenomicRegion& region, svabaReadVector& bav_this, std::vector<AlignedContig>& master_alc, 
  ...
  // set the contig uid
  std::string name = opt::analysis_id + ".c_" + std::to_string(region.chr+1) + "_" + std::to_string(region.pos1) + "_" + std::to_string(region.pos2);
  ...
  for (auto& i : all_contigs_this) {
    ...
    std::set<std::string> fpa_contigs; // store fpa hits to avoid computing multiple times

    if (opt::fpa)
    {
        for (auto& i : ct_alignments) {

            // this contig has already been analysed
            if(fpa_contigs.find(i.Qname())!=fpa_contigs.end())
                continue;

            // any easier way to get the chrom ID as a string?
            std::stringstream chrss (i.ChrName(bwa_header));
            int chrint = 0;
            chrss >> chrint;
            std::string chr_name = main_bwa->ChrIDToName(chrint);

            // overlaps with the target region
            if(region.GetOverlap(SeqLib::GenomicRegion(i.ChrID(), i.Position(), i.PositionEnd())) > 0) {

                int sp = i.Position();
                int ep = i.PositionEnd()-1;
 		
		//std::cout<<sp<<" "<<ep<<" : "<<region.pos1<<" "<<region.pos2<<std::endl;

                std::string refseq = refg->QueryRegion( chr_name, sp, ep);

                if(i.ReverseFlag())
                    refseq = reverseComplement(refseq);

                std::string qryseq = i.Sequence();

                // string comparison
                if(refseq==qryseq) {
                    if (opt::verbose > 1)  { std::cerr << "....fragment " << i.Qname() <<" matches the reference" << std::endl; }

                // or counting the differences
                } else {
                    int diff_pos=100;
                    if(refseq.length()==qryseq.length()) {
                        diff_pos=0;
                        for(int p=0;p<refseq.length();p++){
                            if(refseq.at(p)!=qryseq.at(p))
                                diff_pos++;
                        }
                    }

                    // require minimum X differences
                    if(diff_pos>=opt::fpa_min_better) {
                        if (opt::verbose > 1)  { std::cerr << "....fragment " << i.Qname() <<" will be analysed" << std::endl; }

                        SeqLib::Cigar icig = i.GetCigar();
                        std::string qry;
			//std::cout<<icig<<std::endl;
                        for (SeqLib::Cigar::iterator icig_it = icig.begin(); icig_it != icig.end(); ++icig_it)
                        {
//			    std::cout<<*icig_it<<std::endl;
//			    std::cout<<icig_it.Type();

                            if (icig_it->ConsumesQuery()) {
                                qry += qryseq.substr(0,icig_it->Length());
                                qryseq = qryseq.substr(icig_it->Length());
                            } else {
                                qry += std::string(icig_it->Length(),'-');
                            }
                        }
//			std::cout<<qry<<std::endl;
                        refseq = refg->QueryRegion( chr_name, region.pos1-opt::fpa_ref_flank, region.pos2+opt::fpa_ref_flank-1);
//			std::cout<<refseq<<" "<<chr_name<<" "<<region.pos1<<" "<<region.pos2<<std::endl;
                        int cs=0; int ce = 0;
                        if(sp < region.pos1-opt::fpa_ref_flank)
                            cs = (region.pos1-opt::fpa_ref_flank)-sp;

                        if(ep > region.pos2+opt::fpa_ref_flank-1)
                            ce = ep-(region.pos2+opt::fpa_ref_flank-1);

                        qry = qry.substr(cs,qry.length()-cs-ce);
                        qry.erase(std::remove(qry.begin(), qry.end(), '-'), qry.end());

                        qryseq = qry;

			if(qryseq.length()>0)
	                    run_fpa(refseq,qryseq,&i,&fpa_contigs,refg,region);

                    } else if (opt::verbose > 1)  {
                        std::cerr << "....fragment " << i.Qname() <<" differs only at "<< diff_pos<<" position(s)" << std::endl;
                    }
                }
            }
            // overlaps with the target region
...
void run_fpa(std::string refseq, std::string qryseq, SeqLib::BamRecord *hit, std::set<std::string> *fpa_contigs, SeqLib::RefGenome* refg,const SeqLib::GenomicRegion& region)
{
    if(opt::verbose > 2)
        std::cout<<"R: " <<refseq<<"\nQ: "<<qryseq<<std::endl;

    std::vector<FPA::seqCoordinate> path;

    std::vector<FPA::switchPoint> points;
    for(int i=0;i<4;i++)
    {
        FPA::switchPoint sp;
        points.push_back(sp);
    ...
    std::vector<FPA::seqCoordinate> fwd_path;

    FPA fpa;

    fpa.build_sequences(&qryseq,&refseq);

    fpa.align_sequences(&path,&points);
    fpa.fwd_align_sequences(&fwd_path);
    fpa.compute_fpa_score(&path,&points,&fwd_path);

    if(opt::fpa_debug)
    {
        fpa.set_fpa_debug(true);
        fpa.set_fpa_long_output(true);
    }


    float flank_sim_lim = opt::fpa_flank_sim;
    float frag_sim_lim = opt::fpa_frag_sim;
    int min_better = opt::fpa_min_better;
    int this_better =
            ( fpa.get_fwd_sum_del()+fpa.get_fwd_sum_ins()+fpa.get_fwd_sum_mis() ) -
                ( fpa.get_inv_sum_del()+fpa.get_inv_sum_ins()+fpa.get_inv_sum_mis() );
    int nuc_lim = 3;
    if(  fpa.get_inv_length(&points) >= opt::fpa_frag_length
            && fpa.get_up_ident() > flank_sim_lim && fpa.get_inv_ident() > frag_sim_lim
            && fpa.get_down_ident() > flank_sim_lim
            && fpa.get_sum_nuc() > nuc_lim && this_better>=min_better)
    {
        std::stringstream chrss (hit->ChrName(bwa_header));
        int chrint = 0;
        chrss >> chrint;
        std::string chr_name = main_bwa->ChrIDToName(chrint);

        int frag_start = region.pos1-opt::fpa_ref_flank-1;

        int ref_sp1 = frag_start+fpa.get_ref_sp1(&points)+1;
        int ref_sp2 = frag_start+fpa.get_ref_sp2(&points)+1;
        int ref_sp3 = frag_start+fpa.get_ref_sp3(&points)+1;
        int ref_sp4 = frag_start+fpa.get_ref_sp4(&points)+1;

        int qry_sp1 = fpa.get_qry_sp1(&points);
        int qry_sp2 = fpa.get_qry_sp2(&points);
        int qry_sp3 = fpa.get_qry_sp3(&points);
        int qry_sp4 = fpa.get_qry_sp4(&points);

        os_fpaout << chr_name <<" "<< ref_sp1 <<" "<< ref_sp2 <<" "<< ref_sp3 <<" "<< ref_sp4 <<" "<< qryseq
                  <<" | " << hit->Position() <<" "<< hit->PositionEnd()-1 <<" "<< hit->CigarString()
                  <<" | " << qry_sp1 <<" "<< qry_sp2 <<" "<< qry_sp3 <<" "<< qry_sp4 <<  std::setprecision(3)
                  <<" | " << fpa.get_inv_length(&points) <<" "<< fpa.get_sum_nuc()
                  <<" "   << fpa.get_fwd_sum_del() <<" "<< fpa.get_fwd_sum_ins() <<" "<< fpa.get_fwd_sum_mis()
                  <<" "   << fpa.get_inv_sum_del() <<" "<< fpa.get_inv_sum_ins() <<" "<< fpa.get_inv_sum_mis()
                  <<" "   << fpa.get_up_ident() <<" "<< fpa.get_repeat_ident() <<" "<< fpa.get_down_ident()
                  <<" | " << fpa.get_inv_ident() <<" "<< fpa.get_fwd_ident()
                  << std::endl;
	os_fpaout.flush();

        fpa_contigs->insert(hit->Qname());

        std::cout<< fpa.get_fwd_sum_del()<<" "<<fpa.get_fwd_sum_ins()<<" "<<fpa.get_fwd_sum_mis()<<" "<<fpa.get_inv_sum_del()<<" "<<fpa.get_inv_sum_ins()<<" "<<fpa.get_inv_sum_mis()<<"\n";
        if(opt::fpa_print)
        {
            if (opt::verbose > 2)
            {
                std::cerr <<" "<< refg->QueryRegion( chr_name, ref_sp1-20, ref_sp1-1 ) <<"|"<<
                         reverseComplement( refg->QueryRegion( chr_name, ref_sp3-1, ref_sp2-1 ) ) <<"|"<<
                         refg->QueryRegion( chr_name, ref_sp4-1, ref_sp4+18 ) << std::endl;

                fpa.set_fpa_long_output(true);
            }
            fpa.print_inversion_fragment(&points);
            fpa.print_switch_process(&path,&points);
        }
    }
}
